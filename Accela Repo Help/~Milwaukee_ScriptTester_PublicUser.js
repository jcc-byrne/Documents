...
try {
  ...
  showDebug = true;
  publicUser = true;
  publicUserID = 'PUBLICUSER108837'


  var businessContact = getContactByType('Business Contact', capId);

  var csm = aa.people.getCapContactByPK(capId,businessContact.getContactSeqNumber()).getOutput()
  var bcObject = new contactObj(csm)

  exploreObject(bcObject)

  //get public user and check to see if the contact is missing 
  ...
  if (getUserResult.getSuccess() && getUserResult.getOutput()) {
      ...
      logDebug("Couldn't link public user for, no such public user"); 
  }
  ...
  for (var y in objExplore) {
    try {
      if (typeof objExplore[y] !== 'function') {
        logDebug('<font color=red>  <b> ' + y + ': </b></font> ' + objExplore[y]);
      }
    } catch (err) {
