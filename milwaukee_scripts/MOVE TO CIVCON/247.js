...
logDebug('>>>START of 247')
if (appMatch('Licenses/Business/Cigarette & Tobacco/Application') && !feeExists('LIC_CIG03')){

    if (!matches(AInfo['Months to Prorate Fee'], null, undefined, '')){
        logDebug('Prorate Months: ' + AInfo['Months to Prorate Fee']);
        licFeeAmt = feeAmount('LIC_CIG02');
        logDebug('License Fee Amount: ' + licFeeAmt)
        if (licFeeAmt){
            feeMsg = 'Fee Prorated by ' + AInfo['Months to Prorate Fee'] + ' to align all licenses to the same expiration date';
            feeQty = (licFeeAmt / 12 ) * AInfo['Months to Prorate Fee'];
            addFeeWithExtraData('LIC_CIG03', 'LIC_CIG_APP', 'FINAL', feeQty, 'Y', capId, feeMsg, null, null)
        }
    }
    logDebug('<<<End of 247')
} else {
logDebug('Fee already exists');
  logDebug('<<<End of 247')
}
