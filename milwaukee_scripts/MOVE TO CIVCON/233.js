...
if (appMatch('Licenses/Business/*/*') || appMatch('Licenses/Mobile Vendor/*/*') || appMatch('Licenses/Parking/*/*') || appMatch('Licenses/Amusement/*/*') || appMatch('Licenses/Transportation/*/*') || appMatch('Licenses/Temporary/*/*') || appMatch('Licenses/Amendment/*/*') ||appMatch('ABC/*/*/*') ){

    if( cap.isCompleteCap()){
        logDebug('>>>Start of 233')

        var businessEntity = getContactByType('Business Entity', capId);
        var businessContact = getContactByType('Agent-Sole Proprietor-1st Parter', capId)

        var eParams = aa.util.newHashtable();
        var eNotification = 'LIC_FEES_INVOICED'
        var emailTo = ''
        
        businessEntity.getEmail != null ? emailTo += businessEntity.getEmail() + '; ' : '';
        businessContact.getEmail != null ? emailTo += businessContact.getEmail() : '';

        getRecordParams4Notification(eParams, capId)
        // TODO update this for PROD
        var theURL = "https://aca-civcon.accela.com/milwaukee";
        addParameter(eParams, "$$ACAURL$$", getACARecordURL(theURL));
        sendNotification('', emailTo, '', eNotification, eParams, []);
