...
try {
  ...
  try {
    ...
    var pCapIds = aa.cap.getProjectParents(capId, 1)

    if (pCapIds.getSuccess()){
        pCapIds = pCapIds.getOutput();

        for (i in pCapIds){
            logDebug(pCapIds[i].getCapModel().getAltID())
            // exploreObject(pCapIds[i].getCapModel().getAltID())
