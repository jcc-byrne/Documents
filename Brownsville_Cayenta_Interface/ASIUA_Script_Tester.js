...
try {
  ...
  if(isTaskActive('Inspection') && getAppSpecific('Provider') == 'BPUB' && (asiTempPole == 'Yes' || asiDisconnect == 'Yes' || asiWorkingClearance == 'Yes')){
    ...
    var projectCreated = true;
    var bpubTable = loadASITable('BPUB')

    for (i in bpubTable){
      bpubRow = bpubTable[i]
      if (bpubRow['Connection TP'] == 'PROJECT' && bpubRow['Permit Number'] == 'RESEND'){
    ...
    if (projectCreated){
      var resendArr = getFailedASITrows();
      for (row in resendArr){
          tblRow = resendArr[row];
          rowIndex = tblRow[0];
          connectionTp = tblRow[1]
          resendPermitEntry(prjNo, String(connectionTp), rowIndex)
      }
    } else if (!projectCreated){
      var resendArr = getFailedProjectRow();

      for (row in resendArr){
        tblRow = resendArr[row];
        rowIndex = tblRow[0];
        connectionTp = tblRow[1]
        resendPermitProject(prjNo, prjFdesc, prjSdesc, prjExpDD, prjComments, rowIndex)
    }
