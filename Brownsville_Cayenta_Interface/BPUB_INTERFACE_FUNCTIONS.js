function addPermitProject(projectNo, projectFdesc, projectSdesc, rejectionDt, comments, itemCap){
    ...
    if (res.getSuccess()){
        ...
        if (resStatus == '-1'){
            ...
            createComment('An error occured attempting to create BPUB Project: ' + statusDesc, itemCap)
        responseStatusComment = 'ERROR creating Project... Description: ' + statusDesc;
        newRow['Permit Number'] = new asiTableValObj('Permit Number', 'RESEND', 'N')
        newRow['Connection TP'] = new asiTableValObj('Connection TP', 'PROJECT', 'N')
        newRow['Status'] = new asiTableValObj('Status', '', 'N')
        newRow['Response Status'] = new asiTableValObj('Response Status', responseStatusComment, 'Y')
        addToASITable('BPUB',newRow, capId)
            ...
            createComment('BPUB Project, ' + projectNo + ' created successfully: ' + statusDesc, itemCap)
            newRow['Permit Number'] = new asiTableValObj('Permit Number', projectNo, 'Y')
            newRow['Connection TP'] = new asiTableValObj('Connection TP', 'PROJECT', 'Y')
            newRow['Status'] = new asiTableValObj('Status', '', 'Y')
            responseStatusComment = 'BPUB Project, ' + projectNo + ' created successfully: ' + statusDesc;
            newRow['Response Status'] = new asiTableValObj('Response Status', responseStatusComment, 'Y')
            addToASITable('BPUB',newRow, capId)
            aa.print('>>> END addPermitProject')
            return true
...
function resendPermitProject(projectNo, projectFdesc, projectSdesc, rejectionDt, comments, rowIndex){
    aa.print('>>> BEGIN addPermitProject')
    var functionName = 'AddPermitProject'
    // var projectNo = '202400013C'; //Accela Permit Number
    // var projectFdesc = '200 Amps (2x), TEMP '; //full city description
    ...
    if (res.getSuccess()){
        ...
        if (resStatus == '0'){
            logDebug('Project Created')
            updateAsitPermitNumber(projectNo, rowIndex)
            responseStatusComment = 'CREATED Project... Description: ' + statusDesc;
            updateASITpermitResStatus(rowIndex, responseStatusComment)
            showDebug = false;
            showMessage = true;
            logMessage('Successfully created BPUB Permit', 'SUCCESS')
            ...
            showDebug = false;
            showMessage = true;
            logMessage('There was an error creating the BPUB Permit, try again', 'WARNING')
        }
    ...
    if (meterInfoASIT){
        ...
        for (i in meterInfoASIT){
            ...
            if (quantity != null && quantity != 0 && quantity != undefined && quantity != ''){
                ...
                // TODO add waiting function

                while (count < quantity){
                    logDebug('Count: ' + count)
                    if (count > 0 && count % 5 == 0){
                        aa.print('wait five seconds because count is == ' + count)
                        sleep(5)
                    }
...
function addPermitEntry(projectNo, connectionTp){
    ...
    if (res.getSuccess()){
        ...
        if (resStatus == '0'){
            ...
            newRow['Permit Number'] = new asiTableValObj('Permit Number', permNo, 'Y')
            ...
            newRow['Permit Number'] = new asiTableValObj('Permit Number', 'RESEND', 'Y')
            responseStatusComment = 'ERROR... Description: ' + statusDesc;
        ...
        newRow['Connection TP'] = new asiTableValObj('Connection TP', connectionTp, 'Y')
        newRow['Status'] = new asiTableValObj('Status', 'ENTER', 'Y')
        newRow['Response Status'] = new asiTableValObj('Response Status', responseStatusComment, 'Y')
        addToASITable('BPUB',newRow, capId)
    }
...
function getBPUBProjectFDesc() {
    ...
    for (i in meterInfoASIT){
        ...
        if (connectionTp == 'HOUSE'){
            connectionTp = 'HSE'
        }

        if (amps != null && amps != ''){

            fDesc+= amps + 'A ' + connectionTp;
            if (quantity > 1){
                fDesc+= ' (X' + quantity + ') ';
            }
        } else {
...
function getASITpermitNumbers(connectionTp) {
    ...
    "FROM b1permit p join bappspectable_value t on t.b1_per_id1 = p.b1_per_id1 and t.b1_per_id2 = p.b1_per_id2 and t.b1_per_id3 = p.b1_per_id3 AND t.TABLE_NAME = 'BPUB' and t.ATTRIBUTE_VALUE = ? \n" +
    ...
    "AND  p.b1_alt_id = ?  \n" 
    var result = aa.db.select(sql,[connectionTp, agency, capId.getCustomID()]);
...
return resultArray
    ...
    function getAllASITPermitNumbers() {
        ...
        var agency = aa.getServiceProviderCode();
        var sql = "SELECT p.b1_alt_id, t.row_index, t.ATTRIBUTE_VALUE as connectionTp, t2.ATTRIBUTE_VALUE as permitnum, t3.ATTRIBUTE_VALUE as permStatus \n" +
        "FROM b1permit p join bappspectable_value t on t.b1_per_id1 = p.b1_per_id1 and t.b1_per_id2 = p.b1_per_id2 and t.b1_per_id3 = p.b1_per_id3 and t.ATTRIBUTE_VALUE in ('HOUSE', 'PERM', 'RELOC', 'SERIN') \n" +
        "JOIN bappspectable_value t2 on t.b1_per_id1 = t2.b1_per_id1 and t.b1_per_id2 = t2.b1_per_id2 and t.b1_per_id3 = t2.b1_per_id3 and t.row_index = t2.row_index and t2.column_name = 'Permit Number' and t2.table_name = 'BPUB' \n"+
        "JOIN bappspectable_value t3 on t.b1_per_id1 = t3.b1_per_id1 and t.b1_per_id2 = t3.b1_per_id2 and t.b1_per_id3 = t3.b1_per_id3 and t.row_index = t3.row_index and t3.column_name = 'Status' and t3.ATTRIBUTE_VALUE in ('ENTER', 'Entered') and t3.table_name = 'BPUB' \n"+
        "WHERE p.SERV_PROV_CODE = ? \n" +
        "AND  p.b1_alt_id = ? \n" 
    ...
    function getFailedASITrows() {
        ...
        "JOIN bappspectable_value t2 on t.b1_per_id1 = t2.b1_per_id1 and t.b1_per_id2 = t2.b1_per_id2 and t.b1_per_id3 = t2.b1_per_id3 and t.row_index = t2.row_index and t2.column_name = 'Connection TP' and t.attribute_value <> 'PROJECT' and t2.table_name = 'BPUB' \n"+
        "WHERE p.SERV_PROV_CODE = ? \n" +
        "AND  p.b1_alt_id = ? \n" 
        var result = aa.db.select(sql,[agency, capId.getCustomID()]);
...
function createComment(vComment, itemCap) //optional CapId, optional vDispOnInsp
{
	var comDate = aa.date.getCurrentDate();
	var capCommentScriptModel = aa.cap.createCapCommentScriptModel();
	capCommentScriptModel.setCapIDModel(itemCap);
	...
	capCommentModel.setDisplayOnInsp("N");
